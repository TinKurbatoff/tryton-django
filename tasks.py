...
def coverage(c):
    ...
    """
    c.run("coverage run --source tryton-django runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/tryton-django.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ tryton_django")
...
def lint(c):
    ...
    """
    c.run("flake8 tryton-django tests")
...
def release(c, bumpsize=''):
    ...
    import tryton_django
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=tryton_django.__version__))
    c.run("git push --tags")
